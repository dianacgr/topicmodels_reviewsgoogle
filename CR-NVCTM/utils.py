...
def save_topics(doc_word, topic_word, N):
...
def printTopics(model='cr_nvctm', url='/data/Snippets/train.feat', vocab_size=30642):
    with codecs.open('./{}_train_beta'.format(model), 'rb') as fp:
        beta = pickle.load(fp)
    fp.close()
    test_mat = data_set(url)
    top_n = [10]
    for n in top_n:
        save_topics(test_mat, np.array(beta), n)